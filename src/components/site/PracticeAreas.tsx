...
import {
  HeartPulse, Users, Briefcase, Scroll, Shield, Home, ArrowUpRight,
} from "lucide-react";
...
export function PracticeAreaGrid() {
  return (
    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {PRACTICE_AREAS.map(({ slug, title, icon: Icon, desc }) => (
        <Link
          key={slug}
          to="/practice-areas"
        ...
        >
          ...
          <span className="inline-flex items-center gap-1.5 text-sm font-medium text-[var(--color-navy)] group-hover:text-[var(--color-gold)]">
